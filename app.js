...
(function(){
  ...
  unitsSel.addEventListener('change', async ()=>{ current.units=unitsSel.value; await saveDay(current); updateSugars(); drawChart(); });
  ...
  function displayValue(mgdl){ if(mgdl==null||mgdl===undefined||mgdl==='') return ''; return (unitsSel.value==='mmol')? (Number(mgdl)/18).toFixed(1) : String(mgdl); }
